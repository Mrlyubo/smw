    		...
    		//If there are two words, assume they are first and last names respectively
    			...
    			if(count($names) == 3)
    				$usersReturnedQuery = mysqli_query($con, "SELECT * FROM users
                        WHERE (first_name LIKE '$names[0]%' AND last_name LIKE '$names[2]%') AND user_closed='no'");
    			...
    			if($user['username'] != $row['username']) {
    				...
    				if(isset($_POST[$row['username']])) {
    					...
    					if($user_obj->isFriend($row['username'])) {
    						...
    						header("Location: http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]");//refresh the current page.
    					}
    					else if($user_obj->didReceiveRequest($row['username'])) {
