...
class Post {
    ...
    public function loadPostsFriends($data, $limit){
		...
		if(mysqli_num_rows($data_query) > 0) {
            ...
            while($row = mysqli_fetch_array($data_query)){
                ...
                //Check if user who posted, has their accont closed
                ...
                if($user_logged_obj->isFriend($added_by)){
                    ...
                    //Once 10 posts have heen loaded, break
                    if($count > $limit){
                        break;
                    }else{
                        $count++;
                    ...
                    $interval = $start_date->diff($end_date);//Difference between $start_date and $end_date
                    if($interval->y >=1){
                        if($interval->y == 1){
        ...
        }//End if(mysqli_num_row($data_query))
        echo $str;
    }//End public function loadPostsFriends($data, $limit)
    ...
    public function getSinglePost($post_id) {//copy from function loadPostsFriends

    		$userLoggedIn = $this->user_obj->getUsername();

            //Once opened the single post.
